...
async function test4_CreateClient() {
  ...
  if (status === 201 && data.id) {
    ...
    console.log(
      "   Name:",
      `${data.name.firstName} ${data.name.firstLastName}`,
    );
...
async function test9_SearchByName() {
  ...
  const { status, data } = await apiCall(
    "GET",
    "/api/v1/clients/search?q=Francisco",
  );

  if (status === 200 && Array.isArray(data.data)) {
    const found = data.data.some((c) =>
      c.name.firstName.toLowerCase().includes("francisco"),
    );
    if (found) {
...
async function test10_SearchByFullName() {
  ...
  const { status, data } = await apiCall(
    "GET",
    "/api/v1/clients/search?q=Francisco%20Noya",
  );
