...
jest.mock('@/components/transactions/transaction-audit-history', () => ({
  TransactionAuditHistory: () => <div data-testid="transaction-audit-history">Audit History Component</div>,
}));
...
describe('TransactionDetailPage', () => {
  ...
  it('renders transaction details after loading', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Points earned from purchase')).toBeInTheDocument(); // Description
    });
  });
  ...
  it('navigates back to list on back button click', async () => {
    ...
    const backButton = document.querySelector('button.h-4.w-4')?.closest('button');
    if (backButton) {
  ...
  it('navigates to client detail on client ID click', async () => {
    ...
    expect(mockRouter.push).toHaveBeenCalledWith('/dashboard/clients/client-123');
  });
