...
jest.mock('@/components/clients/client-audit-history', () => ({
  ClientAuditHistory: () => <div data-testid="client-audit-history">Audit History Component</div>,
}));
...
describe('ClientDetailPage', () => {
  ...
  it('handles delete action', async () => {
    ...
    await waitFor(() => {
      expect(apiRequest).toHaveBeenCalledWith('/clients/123', expect.objectContaining({
        method: 'DELETE',
      }));
  ...
  it('switches to audit history tab', async () => {
    ...
    // Check that details are visible initially
    expect(screen.getByText('Personal Information')).toBeInTheDocument();
    
    // Click Audit History tab
    const auditTab = screen.getByText('Audit History');
